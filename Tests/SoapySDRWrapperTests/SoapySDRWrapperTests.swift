...
final class SoapySDRWrapperTests: XCTestCase {
    ...
    func testSoapyStreamRead() throws {
        ...
        device.setRxFrequency(channel: 0, frequency: 100_100_000)
        device.setRxGainMode(channel: 0, automatic: true)
        print(device.description)
        ...
        print("\(readCount1) samples in \(timestamp1) nanoseconds, rate: \(Double(readCount1) * (1e9 / Double(timestamp1))) samples/second (timestamp-based)")
    }
    
    func testSoapyAsyncHandler() throws {
        // Parameters
        let readSeconds = 25.0
        let sampleRate: Double = 2_400_000
        
        let semQueue = DispatchQueue.init(label: "testAsyncQueue")
        
        let device = try getFirstDevice()
        device.setRxSampleRate(channel: 0, rate: sampleRate)
